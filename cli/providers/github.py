...
def load_config(path: str | Path | dict, role: str = "") -> GitHubConfig:
    try:
        ...
        return GitHubConfig(
            ...
            config["repository"],
            visibility,
            config.get("membersFile", ""),
            config.get("baseURL", "https://api.github.com"),
        )
    ...
    try:
        ...
        for item in raw.get("members", []):
            ...
            if (
                ...
                or permission not in {"pull", "push", "triage", "maintain", "admin"}
            ):
                raise ValueError(
                    "member requires role, usernames and a valid permission"
                )
            members.extend(
                GitHubMember(role, username, permission)
...
class GitHubClient:
    ...
    def request(self, method: str, path: str, body: dict | None = None) -> dict:
        ...
        try:
            ...
            raise ValueError(
                f"GitHub returned {error.code}: {error.read().decode()}"
            ) from error
