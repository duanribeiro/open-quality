...
def save_state(path: str | Path, state: JiraState) -> None:
    p=Path(path); p.parent.mkdir(parents=True,exist_ok=True); p.write_text(json.dumps({"version":state.version,"provider":state.provider,"target":state.target,"resources":{k:asdict(v) for k,v in state.resources.items()}},indent=2)+"\n")
...
def apply(ops:list[Operation],state:JiraState,client:JiraClient,config:JiraConfig)->JiraState:
 for op in ops:
  ...
  try:
   ...
   elif op.kind=="KanbanBoard": eid=0; href="/rest/agile/1.0/board"
