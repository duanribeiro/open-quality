    ...
    def test_apply_only_provisions_members(self) -> None:
        ...
        try:
            ...
            with patch.object(gitlab.GitLabConfig, "token", return_value="token"):
                gitlab.apply(
                    config,
                    [gitlab.GitLabMember("software-engineer", "octocat", "developer")],
                )
        finally:
            gitlab.GitLabClient = original

        client.invite_member.assert_called_once_with(
            42, gitlab.GitLabMember("software-engineer", "octocat", "developer")
        )
